...
class TrainConfig:
    ...
    tensorboard_dir: str = r"./runs/resnet_vae"
...
def train_stage1(model, train_loader, val_loader, loss_fn, config, writer):
    ...
    for name, param in model.named_parameters():
        # Decoder相关参数：post_quant_conv、decoder
    ...
    for epoch in range(config.num_epochs_stage1):
        ...
        if (epoch + 1) % config.save_interval == 0:
            save_path = os.path.join(config.save_dir, f"resnet_stage1_epoch_{epoch + 1}.pth")
            save_model(model, optimizer, epoch + 1, val_losses["total_loss"], current_step, save_path)

    # 保存阶段1最终模型
    final_save_path = os.path.join(config.save_dir, "resnet_stage1_final.pth")
...
def train_stage2(model, train_loader, val_loader, loss_fn, config, start_step, writer):
    ...
    for epoch in range(config.num_epochs_stage2):
        ...
        # 保存模型
        if (epoch + 1) % config.save_interval == 0:
            save_path = os.path.join(config.save_dir, f"resnet_stage2_epoch_{epoch + 1}.pth")
            save_model(model, optimizer, epoch + 1, val_losses["total_loss"], current_step, save_path)

    # 保存阶段2最终模型
    final_save_path = os.path.join(config.save_dir, "resnet_stage2_final.pth")
...
if __name__ == "__main__":
    ...
    print(f"训练配置 | 设备: {config.device} | 批次大小: {config.batch_size} | "
          f"模型类型: {config.model_type} | 潜在空间通道数: {config.latent_channels} | "
    ...
    if config.model_type == "resnet50":
        model = ResNet50Autoencoder(latent_channels=config.latent_channels).to(config.device)
    elif config.model_type == "resnet18":
        model = ResNet18Autoencoder(latent_channels=config.latent_channels).to(config.device)
    else:
        raise ValueError(f"不支持的模型类型: {config.model_type}，请选择 'resnet50' 或 'resnet18'")
